...
| **MARV-69** LLVM optimized release backend *(in progress — `native-llvm` emits deterministic LLVM IR and runs/links through `clang -O2` for scalar/calls/recursion/conditionals/loops, boxed structs/enums, arrays, runtime slice updates, `List[T]`, string ops, iterator loops, and current map/set corpus paths; `raise`, capability `perform`, unsafe/resource host integration, and broader app/parser paths remain unsupported until later slices)* | 7 · Backends | 68 | — | medium |
