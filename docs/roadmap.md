...
|------|-------|-----------|----------|----------|
| **MARV-1** enums + `match` (payloads) | 1 · Surface (spine) | — | 3, 5, 9, `std` | high |
...
- **Phase 1 · Surface.** Grow what the parser/lowering accept. The single biggest unblocker;
  most other work compounds on it. Today's parsed subset: `mod`/`import`, `struct`/`fn`
  (incl. `pure fn`), `let`/`var`, `if`/`else`, the binary operators, calls/recursion, field
