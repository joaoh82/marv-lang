...
| **MARV-69** LLVM optimized release backend *(in progress — first `native-llvm` slice emits deterministic LLVM IR and runs/links through `clang -O2` for scalar/calls/recursion/conditionals/loops, boxed structs/enums, arrays, and runtime slice updates; list/map/set runtime helpers, string builders, `raise`, capabilities, and unsafe/resource host integration remain unsupported until the next slice)* | 7 · Backends | 68 | — | medium |
