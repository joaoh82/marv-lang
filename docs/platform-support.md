...
| LLVM (release) | `marv-codegen-llvm` | textual LLVM IR compiled/linked by `clang` | **first slice working** for scalar arithmetic/casts, calls/recursion, `if`/`match`, `while`, early `return`, boxed structs/enums, arrays, runtime-length slice updates, `List[T]`, strings, iterator loops, and current map/set corpus paths; `raise`, capability `perform`, unsafe/resource host integration remain honest `unsupported` |
