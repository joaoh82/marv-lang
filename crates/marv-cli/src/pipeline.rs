...
pub struct Loaded {
    ...
    pub runtime_defs: Vec<(Hash, String, Def)>,
    pub runtime_aliases: Vec<(String, Hash)>,
...
fn load_source(src: &str, path: &Path) -> Result<Loaded, LoadError> {
    ...
    // The user's file is module 0; `check` operates on its definitions (the std
    // modules are resolved-but-trusted library code). `run`/`build` retain the
    // resolved std bodies above for source-level std helpers.
...
) -> (Vec<(Hash, String, Def)>, Vec<(String, Hash)>) {
